...
- points   … 要点。多くても6項目まで、1項目は30〜40字程度の短い一文で。
             「〜について検討した」のような1文に多くの情報を詰め込んだ
             長い文にせず、要点だけを短く切り出してください（例:
             「機械系エンジニアが有力候補として浮上」であって、なぜ・
             どんな数字だったかまでは書かない）。細かい経緯や数字は
             拾いすぎず、後で読んで話の流れが分かる粒度に絞ってください。
             相槌（「はい」「うんうん」など）や言い淀みは無視します。
             **実質的な内容のある会話であれば、pointsが空になることは
             まずありません。** 空にしてよいのは、雑談だけで中身が
             無かったときだけです。
  ...
  input_schema: {
    ...
    properties: {
      ...
      points: {
        ...
        items: { type: "string" },
        description:
          "要点（最大6項目）。1項目は30〜40字程度の短い一文にする(長い説明文にしない)。相槌は無視するが、実質的な内容のある会話なら空配列にしないこと。",
      },
